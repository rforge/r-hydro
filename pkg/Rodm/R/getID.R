...
# Generate Table where to search for information - this could be done once only
	...
	for(i in 1:length(uvalue)){
		...
		if(NROW(entry)==0){
			...
			for(word in words){
				for(field in lookup[[table]]){
					the.dist <- c(the.dist, levenshtein.distance(source=tolower(word), targets=tolower(all.table[[field]])))
				}
			}
			
			#return best matches
			if(length(the.dist)<10){
			...
			cat("\nNo match found for ",uvalue[i],". Fuzzy search returns:\n")
