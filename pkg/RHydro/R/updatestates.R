updatestates <- function(smodl,mparam,dparam,state1) {
   # Update the state variables "after" solving the differential equations
   ...
   if (smodl$arch2 == 32) {  # (lower layer architecture)
     ...
     free_2a <- state1[r,8]                    # primary reservoir
     free_2b <- state1[r,9]                    # secondary reservoir
     free_2  <- free_2a + free_2b              # free storage
     tens_2  <- state1[r,7]                    # tension storage
