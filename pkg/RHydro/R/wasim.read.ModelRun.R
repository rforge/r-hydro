wasim.read.ModelRun <- function(files, data.types=wasim.data.types){
   ...
   fileNr <- 1
   measuredFluxes <- modelledFluxes <- measuredStates <-
          parameters <-  modelledStates <- list()
   for(file in files){
       pars <- wasim.read.ModelParameters(file)
       #ToDo convert parameters into usefull HydroWasimParameters
       parameters[fileNr] <- new("HydroWasimParameters", parameters=data.frame(1) )
       my.ts <- list()
       i <- 1
       while(i <= NROW(data.types)){
           data.types[i,]
           ...
           ts <- wasim.file2HydroTS(file=paste(dirname(file),
data.file, sep="/"), is_recorded=data.types[i,"is_recorded"], is_flux=data.types[i,"is_flux"], units= data.types[i,"units"], type=data.types[i,"type"])
           if(!is.null(ts)){
       ...
       #ToDo use data types
       browser()
       #loop through entries and use [[]] notation to create list with
       #symbols. check for duplicates and create dummy symbols for
       #unknown data.types
